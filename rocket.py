...
class Rocket(object):
    ...
    def __init__(self, max_steps, task='hover', rocket_type='falcon',
        ...
        # target point
        if self.task == 'hover':
            self.target_x, self.target_y, self.target_r = 100, 300, 50
    ...
    def create_polygons(self):
        ...
        if self.task == 'hover':
            ...
            polys['target_region'].append({'pts': pts1, 'face_color': None, 'edge_color': (0, 0, 242)})
            polys['target_region'].append({'pts': pts2, 'face_color': None, 'edge_color': (0, 242, 242)})
        else:
